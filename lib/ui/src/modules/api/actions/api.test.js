...
describe('manager.api.actions.api', () => {
  describe('setStories', () => {
    describe('no selected story', () => {
      it('should set stories and select the first story', () => {
        ...
        expect(newState).toEqual({ stories, selectedKind: 'abc', selectedStory: 'a' });
      });
    });
    ...
    describe('has a selected story', () => {
      it('should set stories and select the existing story', () => {
        ...
        const state = { selectedKind: 'abc', selectedStory: 'c' };
        const newState = clientStore.updateCallback(state);
        expect(newState).toEqual({ stories, selectedKind: 'abc', selectedStory: 'c' });
      });
    });
    ...
    describe('has a selected story, but it is story is not in new stories', () => {
      it('should set stories and select the first story of the selected kind', () => {
        ...
        const state = { selectedKind: 'bbc', selectedStory: 'k' };
        const newState = clientStore.updateCallback(state);
        expect(newState).toEqual({ stories, selectedKind: 'bbc', selectedStory: 'x' });
      });
    });
    ...
    describe('has a selected story, but it is kind is not in new stories', () => {
      it('should set stories and select the first story', () => {
        ...
        const state = { selectedKind: 'kky', selectedStory: 'c' };
        const newState = clientStore.updateCallback(state);
        expect(newState).toEqual({ stories, selectedKind: 'abc', selectedStory: 'a' });
      });
    });
  ...
  describe('selectStory', () => {
    describe('with both kind and story', () => {
      it('should select the correct story', () => {
        ...
        const state = { stories, selectedKind: 'abc', selectedStory: 'c' };
        const stateUpdates = clientStore.updateCallback(state);
        expect(stateUpdates).toEqual({ selectedKind: 'bbc', selectedStory: 'y' });
      });
    });
    ...
    describe('with just the kind', () => {
      it('should select the first of the kind', () => {
        ...
        const state = { stories, selectedKind: 'abc', selectedStory: 'c' };
        const stateUpdates = clientStore.updateCallback(state);
        expect(stateUpdates).toEqual({ selectedKind: 'bbc', selectedStory: 'x' });
      });
    });
  });
  ...
  describe('jumpToStory', () => {
    describe('has enough stories', () => {
      ...
      it('should select the prev story', () => {
        ...
        const state = { stories, selectedKind: 'abc', selectedStory: 'c' };
        const stateUpdates = clientStore.updateCallback(state);
        expect(stateUpdates).toEqual({ selectedKind: 'abc', selectedStory: 'b' });
      });
    });
    ...
    describe('has not enough stories', () => {
      it('should select the current story', () => {
        ...
        const state = { stories, selectedKind: 'bbc', selectedStory: 'z' };
        const stateUpdates = clientStore.updateCallback(state);
        expect(stateUpdates).toEqual({ selectedKind: 'bbc', selectedStory: 'z' });
      });
    });
  });
  ...
  describe('setOptions', () => {
    it('should update options', () => {
      ...
      const state = { uiOptions: { bbc: 50, abc: 40 } };

      const stateUpdates = clientStore.updateCallback(state);
      expect(stateUpdates).toEqual({ uiOptions: { bbc: 50, abc: 10 } });
    });

    const provider = { getPanels: () => ({ 'storybook/actions/actions-panel': { title: 'Action logger' }, 'storybooks/storybook-addon-knobs': { title: 'Knobs' } }) };

    it('should update selectedAddonPanel', () => {
      const clientStore = new MockClientStore();
      actions.setOptions({ clientStore, provider }, { selectedAddonPanel: 'storybooks/storybook-addon-knobs' });

      const state = { uiOptions: {}, selectedAddonPanel: 'storybook/actions/actions-panel' };
  ...
  describe('setQueryParams', () => {
    it('shodul update query params', () => {
      ...
      const state = { customQueryParams: { bbc: 'bbb', abc: 'sshd' } };

      const stateUpdates = clientStore.updateCallback(state);
      expect(stateUpdates).toEqual({ customQueryParams: { bbc: 'bbb', abc: 'aaa', cnn: 'ccc' } });
    });
    ...
    it('should delete the param if it is null', () => {
      ...
      const stateUpdates = clientStore.updateCallback(state);
      expect(stateUpdates).toEqual({ customQueryParams: { bbc: 'ccc' } });
    });
  });
});
