...
describe('manager.api.config.initApi', () => {
  it('should expose correct API methods', done => {
    const actions = { api: { setStories: jest.fn(), selectStory: jest.fn(), setQueryParams: jest.fn() }, shortcuts: { handleEvent: jest.fn() } };

    const clientStore = { subscribe: jest.fn() };

    const provider = { handleAPI(api) {
  ...
  it('should trigger the onStory callback', done => {
    ...
    const clientStore = { subscribe: jest.fn(), getAll: () => ({ selectedKind, selectedStory }) };
  ...
  describe('getQueryParam', () => {
    it('should return the correct query param value', done => {
      ...
      const clientStore = { subscribe: jest.fn(), getAll: () => ({ customQueryParams: { foo: 'foo value', bar: 'bar value' } }) };
