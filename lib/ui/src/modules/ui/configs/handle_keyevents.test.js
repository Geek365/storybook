...
describe('manager.ui.config.handle_keyevents', () => {
  test('should call the correct action', () => {
    const actions = { shortcuts: { handleEvent: jest.fn() } };
    const originalOnkeydown = window.onkeydown;
    handleKeyEvents(actions);

    const e = { ctrlKey: true, shiftKey: true, keyCode: keycode('F'), preventDefault() {}, target: { tagName: 'DIV', getAttribute() {
          return null;
        } } };
    window.onkeydown(e);
  ...
  test('should not call any actions if the event target is an input', () => {
    ...
    const e = { ctrlKey: true, shiftKey: true, keyCode: keycode('F'), preventDefault() {}, target: { tagName: 'INPUT', getAttribute() {
          return null;
        } } };
    window.onkeydown(e);
  ...
  test('should not call any actions if the event target has contenteditable enabled', () => {
    ...
    const e = { ctrlKey: true, shiftKey: true, keyCode: keycode('F'), preventDefault() {}, target: { tagName: 'DIV', getAttribute(attr) {
