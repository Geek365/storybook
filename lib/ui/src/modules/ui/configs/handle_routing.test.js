...
describe('manager.ui.config.handle_routing', () => {
  describe('changeUrl', () => {
    ...
    test('should put the correct URL and state to pushState', done => {
      const state = { selectedKind: 'kk', selectedStory: 'ss', customQueryParams: { customText: 'test' }, shortcutOptions: { goFullScreen: false, showDownPanel: true, showLeftPanel: true, downPanelInRight: true }, selectedAddonPanel: 'pp' };
      const clientStore = { getAll: () => state };
      const url = '?customText=test&selectedKind=kk&selectedStory=ss&full=0&down=1&left=1&panelRight=1&downPanel=pp';

      const pushState = { url, selectedKind: 'kk', selectedStory: 'ss', full: false, down: true, left: true, panelRight: true, downPanel: 'pp', customText: 'test' };
  ...
  describe('handleInitialUrl', () => {
    test('should call the correct action according to URL', () => {
      const actions = { api: { selectStory: jest.fn(), setQueryParams: jest.fn() }, shortcuts: { setOptions: jest.fn() }, ui: { selectDownPanel: jest.fn() } };
      ...
      expect(actions.api.setQueryParams).toHaveBeenCalledWith({ customText: 'teststring' });
    });
  });
});
