...
describe('manager.ui.libs.hierarchy', () => {
  describe('createHierarchy', () => {
    test('should return root hierarchy node if stories are undefined', () => {
      ...
      expect(result).toEqual({ namespaces: [], name: '', isNamespace: true, map: new Map() });
    });
    ...
    test('should return flat hierarchy if kind is not separated', () => {
      const stories = [{ kind: 'some.name.item1', namespaces: ['some.name.item1'], stories: ['a1', 'a2'] }, { kind: 'another.space.20', namespaces: ['another.space.20'], stories: ['b1', 'b2'] }];

      const result = createHierarchy(stories);

      const expected = [['some.name.item1', [{ kind: 'some.name.item1', name: 'some.name.item1', highlight: null, namespaces: ['some.name.item1'], stories: [{ name: 'a1', highlight: null }, { name: 'a2', highlight: null }] }]], ['another.space.20', [{ kind: 'another.space.20', name: 'another.space.20', highlight: null, namespaces: ['another.space.20'], stories: [{ name: 'b1', highlight: null }, { name: 'b2', highlight: null }] }]]];
