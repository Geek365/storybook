...
describe('manager.ui.containers.left_panel', () => {
  describe('mapper', () => {
    test('should give correct data', () => {
      ...
      const uiOptions = { name: 'foo', url: 'bar' };
      const selectStory = () => 'selectStory';
      const toggleShortcutsHelp = () => 'toggleShortcutsHelp';
      const setStoryFilter = () => 'setStoryFilter';
      const props = {};
      const env = { actions: () => ({ api: { selectStory }, ui: { toggleShortcutsHelp, setStoryFilter } }) };
      const state = { storyFilter: null, stories, selectedKind, selectedStory, uiOptions };
      const result = mapper(state, props, env);

      expect(result.storiesHierarchy.map).toEqual(new Map([
          [
            'sk',
            [
              {
    ...
    test('should filter stories according to the given filter', () => {
      const stories = [{ kind: 'pk', stories: ['dd'] }, { kind: 'ss', stories: ['dd'] }, { kind: 'pkr', stories: ['dd'] }];
      ...
      const props = {};
      const env = { actions: () => ({ api: { selectStory }, ui: { toggleShortcutsHelp, setStoryFilter } }) };
      const state = { storyFilter: 'ss', stories, selectedKind, selectedStory, uiOptions };
      const result = mapper(state, props, env);
    ...
    test('should filter and sort stories according to the given filter', () => {
      const stories = [{ kind: 'ss', stories: ['dd'] }, { kind: 'pk', stories: ['dd'] }, { kind: 'pkr', stories: ['dd'] }];
      const selectedKind = 'pk';
      const selectedStory = 'dd';
      const uiOptions = { name: 'foo', url: 'bar', sortStoriesByKind: true };
      const selectStory = () => 'selectStory';
      const toggleShortcutsHelp = () => 'toggleShortcutsHelp';
      const setStoryFilter = () => 'setStoryFilter';
      const props = {};
