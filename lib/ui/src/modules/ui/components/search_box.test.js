...
describe('manager.ui.components.search_box', () => {
  const defaultProps = { showSearchBox: false, onSelectStory: () => undefined, onClose: () => undefined };
  describe('render', () => {
    ...
    test('should format stories', () => {
      const stories = [{ kind: 'a', stories: ['b', 'c'] }];
      const wrap = shallow(<SearchBox {...defaultProps} stories={stories} />);
      const search = wrap.find(FuzzySearch);

      const expectedList = [{ type: 'kind', value: 'a', id: 1 }, { type: 'story', value: 'b', id: 2, kind: 'a' }, { type: 'story', value: 'c', id: 3, kind: 'a' }];
  ...
  describe('events', () => {
    ...
    test('should handle selecting a kind', () => {
      ...
      const onClose = jest.fn();
      const wrap = shallow(<SearchBox {...defaultProps} onSelectStory={onSelectStory} onClose={onClose} />);

      const modal = wrap.find(FuzzySearch);
      modal.simulate('select', { type: 'kind', value: 'a' });
    ...
    test('should handle selecting a story', () => {
      ...
      const modal = wrap.find(FuzzySearch);
      modal.simulate('select', { type: 'story', value: 'a', kind: 'b' });
