...
describe('manager.ui.components.left_panel.stories', () => {
  beforeEach(() => setContext({ clientStore: { getAll() {
  ...
  const initialData = [{ kind: 'some.name.item1', stories: ['a1', 'a2'] }, { kind: 'another.space.z20', stories: ['b1', 'b2'] }];
  ...
  describe('render', () => {
    test('should render stories - empty', () => {
      const wrap = shallow(<Stories storiesHierarchy={createHierarchy([])} selectedKind={''} selectedStory={''} selectedHierarchy={[]} />);
    ...
    test('should render stories', () => {
      const wrap = shallow(<Stories storiesHierarchy={data} selectedKind="b" selectedStory="b2" selectedHierarchy={['b']} />);
    ...
    test('should render stories with hierarchy - hierarchySeparator is defined', () => {
      const wrap = shallow(<Stories storiesHierarchy={dataWithSeparator} selectedKind="another.space.z20" selectedStory="b2" selectedHierarchy={['another', 'space', 'z20']} />);
      ...
      expect(output).toMatch(/z20/);
      expect(output).toMatch(/b1/);
      expect(output).toMatch(/b2/);
    });

    test('should render stories without hierarchy - hierarchySeparator is not defined', () => {
      const wrap = shallow(<Stories storiesHierarchy={dataWithoutSeparator} selectedKind="another.space.z20" selectedStory="b2" selectedHierarchy={['another.space.z20']} />);
      ...
      expect(output).not.toMatch(/a1/);
      expect(output).not.toMatch(/a2/);
      expect(output).toMatch(/another.space.z20/);
      expect(output).toMatch(/b1/);
      expect(output).toMatch(/b2/);
    });
    ...
    test('should render stories with initially selected nodes according to the selectedHierarchy', () => {
      ...
      expect(nodes).toEqual({
        ...
        'another@space@z20@component': true,
      });
    });

    test('should contain state with all selected nodes after clicking on the nodes', () => {
      const wrap = mount(<Stories storiesHierarchy={dataWithSeparator} selectedKind="another.space.z20" selectedStory="b2" selectedHierarchy={['another', 'space', 'z20']} />);
    ...
    test('should recalculate selected nodes after selectedHierarchy changes', () => {
      const wrap = mount(<Stories storiesHierarchy={dataWithSeparator} selectedKind="another.space.z20" selectedStory="b2" selectedHierarchy={[]} />);

      wrap.setProps({ selectedHierarchy: ['another', 'space', 'z20'] });
    ...
    test('should not call setState when selectedHierarchy prop changes with the same value', () => {
      const selectedHierarchy = ['another', 'space', 'z20'];
      const wrap = mount(<Stories storiesHierarchy={dataWithSeparator} selectedKind="another.space.z20" selectedStory="b2" selectedHierarchy={selectedHierarchy} />);
    ...
    test('should render stories with with highlighting when storiesFilter is provided', () => {
      const filter = 'th';
      const selectedKind = 'another.space.z20';

      const filteredData = storyFilter(prepareStoriesForHierarchy(initialData, '\\.'), filter, selectedKind);

      const filteredDataHierarchy = createHierarchy(filteredData);

      const wrap = mount(<Stories storiesHierarchy={filteredDataHierarchy} selectedKind={selectedKind} selectedStory="b2" selectedHierarchy={['another', 'space', 'z20']} storyFilter={filter} />);
  ...
  describe('events', () => {
    test('should call the onSelectStory prop when a collapsed kind is clicked', () => {
      ...
      const wrap = mount(<Stories storiesHierarchy={data} selectedKind="b" selectedStory="b2" selectedHierarchy={['b']} onSelectStory={onSelectStory} />);
    ...
    test('should call the onSelectStory prop when a story is clicked - hierarchySeparator is defined', () => {
      const onSelectStory = jest.fn();
      const wrap = mount(<Stories storiesHierarchy={dataWithSeparator} selectedKind="some.name.item1" selectedStory="a2" selectedHierarchy={['another', 'space']} onSelectStory={onSelectStory} />);

      const a = () => wrap.find('[data-name="another"]').first();
      const b = () => wrap.find('[data-name="space"]').first();
      const c = () => wrap.find('[data-name="z20"]').first();
      ...
      expect(onSelectStory).toHaveBeenCalledWith('another.space.z20', null);
      ...
      // expect(onSelectStory).toHaveBeenCalledWith('another.space.z20', 'b2');
    });
