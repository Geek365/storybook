...
describe('manager.ui.components.left_panel.index', () => {
  test('should render Header and TextFilter by default', () => {
    ...
    const wrap = shallow(<LeftPanel openShortcutsHelp={openShortcutsHelp} storyFilter={storyFilter} />);
  ...
  test('should render stories only if storiesHierarchy prop exists', () => {
    ...
    const wrap = shallow(<LeftPanel storiesHierarchy={storiesHierarchy} selectedKind={selectedKind} selectedStory={selectedStory} selectedHierarchy={['kk']} />);

    const header = wrap.find(Stories).first();
    expect(header.props()).toMatchObject({ storiesHierarchy, selectedKind, selectedStory });
  });
