...
describe('Panel', () => {
  ...
  describe('setKnobs handler', () => {
    ...
    it('should set query params when url params are already read', () => {
      ...
      setKnobsHandler({ knobs, timestamp: +new Date() });
      const knobFromStory = { 'knob-foo': knobs.foo.value, 'knob-baz': knobs.baz.value };
  ...
  describe('handleChange()', () => {
    it('should set queryParams and emit knobChange event', () => {
      const testChannel = { on: jest.fn(), emit: jest.fn() };

      const testApi = { getQueryParam: jest.fn(), setQueryParams: jest.fn(), onStory: jest.fn() };

      const wrapper = shallow(<Panel channel={testChannel} api={testApi} />);

      const testChangedKnob = { name: 'foo', value: 'changed text', type: 'text' };
